...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInUSDT = 50000000;     // 50 USDT
    uint256 public minPriceTicketInUSDT = 5000000;      // 5 USDT
     ...
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with Randomizer Protocol
     */
    constructor(address _usdtTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        usdtToken.safeTransferFrom(address(msg.sender), address(this), amountCakeToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInUSDT += amountCakeToTransfer;
    ...
    function claimTickets(
        ...
        usdtToken.safeTransfer(msg.sender, rewardInUSDTToTransfer);
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInUSDT) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInUSDT - amountToShareToWinners);
     ...
     * It is necessary to wait for the Randomizer response before starting a round.
     ...
     * @param _priceTicketInUSDT: price of a ticket in CAKE
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInUSDT >= minPriceTicketInUSDT) && (_priceTicketInUSDT <= maxPriceTicketInUSDT),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = LotteryInfo({
            ...
            usdtPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
     ...
     * @param _maxPriceTicketInUSDT: maximum price of a ticket in USDT
     */
    function setMinAndMaxTicketPriceInUSDT(uint256 _minPriceTicketInUSDT, uint256 _maxPriceTicketInUSDT)
        external
        onlyOwner
    {
        require(_minPriceTicketInUSDT <= _maxPriceTicketInUSDT, "minPrice must be < maxPrice");

        minPriceTicketInUSDT = _minPriceTicketInUSDT;
        maxPriceTicketInUSDT = _maxPriceTicketInUSDT;
    }
